...
PARAMS: dict = {
    # EMA crossover — primary trend-following signal
    "ema_fast":            12,
    "ema_slow":            26,
    # RSI — momentum confirmation filter
    "rsi_period":          14,
    "rsi_entry_min":       50,   # enter only when RSI confirms upward momentum
    "rsi_exit_overbought": 70,   # exit when overbought
    # Volume filter
    "min_volume_ma":       20,   # bar must have volume > N-bar rolling average
}
...
def compute_signals(df: pd.DataFrame, params: dict) -> tuple[pd.Series, pd.Series]:
    ...
    momentum_ok = rsi > params["rsi_entry_min"]
    ...
    # ── entries & exits ───────────────────────────────────────────────────────
    entries = ema_cross_up & momentum_ok & high_vol
    exits   = ema_cross_down | rsi_cross_ob

    # Fill NaN from indicator warmup with False
